...
| Testes de unidade | `java -cp "out-solid;out-test" TestesMissaoMarte` | **74 verificações, 0 falhas** |
| Teste de integração | `java -cp "out-solid;out-test" TesteIntegracaoConsole` | **17 verificações, 0 falhas** |
| Teste funcional do jogo | `bash test/teste-funcional.sh` | **68 cenários, 0 falhas** |
| Determinismo | suíte executada 3 vezes seguidas | mesmo resultado nas 3 |
| **Total** | | **159 verificações, 0 falhas** |
...
Impacto para manutenção, testes ou evolução: com tudo junto, não havia como exercitar
  ...
  Depois da separação, 159 verificações automatizadas passaram a rodar em menos de um
  segundo.
...
Impacto para manutenção, testes ou evolução: o ponto em que a estrutura se paga é o
  teste. São 159 verificações automatizadas que não existiam e não poderiam existir no
